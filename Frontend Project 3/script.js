document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const upfrontPrices = document.querySelectorAll(".upfront")
    const monthlyPrices = document.querySelectorAll(".monthly")
  
    pricingToggle.addEventListener("change", function () {
      if (this.checked) {
        upfrontPrices.forEach((price) => (price.style.display = "none"))
        monthlyPrices.forEach((price) => (price.style.display = "inline"))
      } else {
        upfrontPrices.forEach((price) => (price.style.display = "inline"))
    ...
    trialForm.addEventListener("submit", (e) => {
      ...
      if (!name || !email || !courseInterest || !terms) {
        alert("Please fill in all required fields and accept the terms.")
        return
      }
    ...
    function animateOnScroll() {
      const elements = document.querySelectorAll(
        ".feature-card, .timeline-content, .mentor-card, .outcome-stat, .pricing-card, .faq-item",
      )
    ...
    // Set initial styles for animation
    document
      .querySelectorAll(".feature-card, .timeline-content, .mentor-card, .outcome-stat, .pricing-card, .faq-item")
      .forEach((element) => {
        element.style.opacity = "0"
        element.style.transform = "translateY(20px)"
        element.style.transition = "opacity 0.5s ease, transform 0.5s ease"
